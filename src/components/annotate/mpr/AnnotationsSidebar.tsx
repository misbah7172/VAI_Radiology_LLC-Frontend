...
export default function AnnotationsSidebar() {
  const { mprAnnotations, series, removeAnnotation } = useMPRStore();

  const totalCount = PLANE_ORDER.reduce((sum, plane) => {
    return sum + Object.values(mprAnnotations[plane]).reduce((s, shapes) => s + shapes.length, 0);
  }, 0);
      ...
      {totalCount > 0 && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '6px 0' }}>
          {PLANE_ORDER.map((plane) => {
            const planeData = mprAnnotations[plane];
            const sliceEntries = Object.entries(planeData)
              .map(([si, shapes]) => ({ sliceIndex: Number(si), shapes }))
            ...
            return (
              <div key={plane} style={{ marginBottom: '4px' }}>
                <div style={{ padding: '5px 12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: color, flexShrink: 0 }} />
                  <span style={{ fontSize: '10px', fontWeight: 700, color: color, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{label}</span>
                  {!hasSeries && <span style={{ fontSize: '9px', color: '#3d3d55', fontStyle: 'italic' }}>(no series)</span>}
                </div>
                {sliceEntries.map(({ sliceIndex, shapes }) => (
                  <div key={sliceIndex}>
                    <div style={{ padding: '2px 12px 2px 20px', fontSize: '9px', color: '#3d3d55', fontWeight: 600, letterSpacing: '0.04em' }}>
                      SLICE {sliceIndex + 1}
                    </div>
                    {shapes.map((shape) => (
                      <div
                        key={shape.id}
                        style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '5px 10px 5px 24px', borderBottom: '1px solid #12121a', transition: 'background 0.15s', cursor: 'default' }}
                      ...
                      >
                        ...
                        <span style={{ fontSize: '11px', fontWeight: 600, color: '#a0a0b2', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {shape.label}
                        </span>
                        <span style={{ fontSize: '8px', color: '#3d3d55', fontStyle: 'italic', flexShrink: 0 }}>{shape.type}</span>
                        <button
